...
class Client extends EventEmitter {
    ...
    */
    async connect() {
        ...
        try {
            ...
            let data = {}
            if(this.options.maxShards === "auto" || (this.options.shardConcurrency === "auto" && this.bot)) {
                data = await this.getBotGateway()
            } else if(!this.options.gatewayURL) {
                data = await this.getGateway()
            } else {
                data = { url: this.options.gatewayURL }
            }
    ...
    */
    createInteractionResponse(interactionID, interactionToken, options, file) {
        ...
        return this.requestHandler.request("POST", Endpoints.INTERACTION_RESPOND(interactionID, interactionToken), true, options, file, `/interactions/${interactionID}/:token/callback`);
    }
