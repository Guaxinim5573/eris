...
class RequestHandler {
    constructor(client, options) {
        ...
        if(options.domain) console.warn("[DEPREACTED] Use domains instead of domain!")
        ...
        this.ratelimits = this.options.domains.reduce((acc, cur) => ({...acc, [cur]: {}}), {});
        this.latencyRef = {
    ...
    */
    request(method, url, auth, body, file, _route, short) {
        ...
        return new Promise((resolve, reject) => {
            let domain = this.routeDomain[route]
            if(!domain) {
                domain = this.options.domains[this.domainIndex++ % this.options.domains.length]
                this.routeDomain[route] = domain
            }
            let attempts = 0;
            ...
            const actualCall = (cb) => {
                    ...
                    if(file) {
                        if(Array.isArray(file)) {
                            ...
                            file.forEach(function(f) {
                                ...
                                data.attach(f.fieldName || f.name, f.file, f.name);
                            });
                            if(body) {
                ...
                try {
                    ...
                    if(!!(Number(domain.charAt(0)))) useHttp = true
                    let port
                    if(domain.includes(":")) {
                        port = domain.split(":")[1]
                        realDomain = domain.split(":")[0]
                    }
                    req = (useHttp ? HTTP : HTTPS).request({
                        method: method,
                        host: realDomain,
                        port: port,
                ...
                req.once("response", (resp) => {
                    ...
                    _respStream.on("data", (str) => {
                        ...
                        if(resp.headers["x-ratelimit-limit"]) {
                            this.ratelimits[domain][route].limit = +resp.headers["x-ratelimit-limit"];
                        }

                        if(method !== "GET" && (resp.headers["x-ratelimit-remaining"] == undefined || resp.headers["x-ratelimit-limit"] == undefined) && this.ratelimits[domain][route].limit !== 1) {
                            this._client.emit("debug", `Missing ratelimit headers for SequentialBucket(${this.ratelimits[domain][route].remaining}/${this.ratelimits[domain][route].limit}) with non-default limit\n`
                        ...
                        this.ratelimits[domain][route].remaining = resp.headers["x-ratelimit-remaining"] === undefined ? 1 : +resp.headers["x-ratelimit-remaining"] || 0;
                        ...
                        if(retryAfter >= 0) {
                            if(resp.headers["x-ratelimit-global"]) {
                                ...
                                this.ratelimits[domain][route].reset = (retryAfter || 1) + now;
                            }
                            ...
                            this.ratelimits[domain][route].reset = Math.max(resetTime - this.latencyRef.latency, now);
                        } else {
                            this.ratelimits[domain][route].reset = now;
                        }

                        if(resp.statusCode !== 429) {
                            const content = typeof body === "object" ? `${body.content} ` : "";
                            this._client.emit("debug", `${content}${now} ${route} ${resp.statusCode}: ${latency}ms (${this.latencyRef.latency}ms avg) | ${this.ratelimits[domain][route].remaining}/${this.ratelimits[domain][route].limit} left | Reset ${this.ratelimits[domain][route].reset} (${this.ratelimits[domain][route].reset - now}ms left)`);
                        }
                        ...
                        if(resp.statusCode >= 300) {
                            if(resp.statusCode === 429) {
                                ...
                                this._client.emit("debug", `${resp.headers["x-ratelimit-global"] ? "Global" : "Unexpected"} 429 (╯°□°）╯︵ ┻━┻: ${response}\n${content} ${now} ${route} ${resp.statusCode}: ${latency}ms (${this.latencyRef.latency}ms avg) | ${this.ratelimits[domain][route].remaining}/${this.ratelimits[domain][route].limit} left | Reset ${delay} (${this.ratelimits[domain][route].reset - now}ms left) | Scope ${resp.headers["x-ratelimit-scope"]}`);
                                if(delay) {
                                    setTimeout(() => {
                                        cb();
                            ...
                            let err;
                            if(response.code) {
                                err = new DiscordRESTError(req, resp, response, stack, domain, route);
                            } else {
                                err = new DiscordHTTPError(req, resp, response, stack, domain, route);
                            }
                            reject(err);
                            return;
                        }
                ...
                req.setTimeout(this.options.requestTimeout, () => {
                    reqError = new Error(`Request timed out (>${this.options.requestTimeout}ms) on ${domain} ${method} ${url}`);
                    req.abort();
                });
            ...
            if(this.globalBlock && auth) {
                this.readyQueue.push(() => {
                    if(!this.ratelimits[domain][route]) {
                        this.ratelimits[domain][route] = new SequentialBucket(1, this.latencyRef);
                    }
                    this.ratelimits[domain][route].queue(actualCall, short);
                });
            } else {
                if(!this.ratelimits[domain][route]) {
                    this.ratelimits[domain][route] = new SequentialBucket(1, this.latencyRef);
